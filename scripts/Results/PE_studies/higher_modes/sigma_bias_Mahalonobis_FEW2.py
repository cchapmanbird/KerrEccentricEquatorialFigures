...
try:
    ...
    for i in tqdm(range(len(param_grid))):
        ...
        #t, p, e, x, pp, pt, pr = kerr_traj(M, mu, a, p0, e0, x0, Phi_phi0=Phi_phi0, Phi_theta0=Phi_theta0, Phi_r0=Phi_r0, T=T_LISA, dt=dt)
    ...
    plt.xlabel('a range',fontsize=20)
    plt.ylabel('e0 range',fontsize=20)
...
Waveform_model = GenerateEMRIWaveform(
            ...
            return_list=False, #False if passing to ResponseWrapper
            )
...
sef_kwargs = {'EMRI_waveform_gen':EMRI_TDI, #EMRI waveform model with TDI response
              ...
              'use_gpu':use_gpu,
              'filename': filename_Fishers, #filename
              'noise_model':get_sensitivity,
              'channels':channels,
              'noise_kwargs':noise_kwargs,
              'plunge_check':False, #we don't check for plunge to save a few extra seconds because we anyway ensure large enough p0.
...
try:
    with h5py.File(f"{filename}/data.h5", "r") as f:
        dist_range = f["dists"][:]  # Read the dataset as a NumPy array
    ...
    dist_range = np.array(dist_range)
    
    #save as an h5py file
    with h5py.File(f"{filename}/data.h5", "a") as f: #append
        f.create_dataset("dists", data=dist_range)
...
if calculate_Fishers:
    for i in tqdm(range(len(param_grid))):
        try:
            ...
            with h5py.File(f"{filename_Fishers}/Fisher_{i}.h5", "r") as f:
                _ = f["Fisher"][:]
            continue
        except FileNotFoundError:
            a = param_grid[i][0]
...
def logmasstransform(Fisher, M, mu, index_of_M = 0, index_of_mu = 1):
    ...
    This makes the Fishers slightly less ill-conditioned. """
...
for i in range(len(param_grid)):
    try:
        ...
        with h5py.File(f"{filename_Fishers}/Fisher_{i}.h5", "a") as f: #'a' mode for appending existing datasets.
...
for j in range(len(alternate_models_list)):
    ...
    mask = np.log10(sigma_contours) < 3.0 #sigma > 1000.0 not expected.
    ...
    #sigma_contours = np.log10(np.array(sigma_contours)) #convert to logscale with the same variable name (easier to switch on / off)
    ...
    #scat_cb.set_label(r"$\log_{10}{{\sigma}}$",fontsize=20)
...
plt.xlabel(r"$\log_{10}{\sigma}$",fontsize=20)
...
for j in range(len(alternate_models_list)):
    ...
    for i in range(len(param_names)):
        if i < len(param_names)//2:
            ...
            axs[0,i].set_xlabel(r"$\sigma($" + rf"{param_names[i]})",fontsize=20)
            ...
            axs[1,i-len(param_names)//2].set_xlabel(r"$\sigma($" + rf"{param_names[i]})",fontsize=20)
...
fig.legend(labels = np.vstack((alternate_models_list_plotting,alternate_models_list_plotting)).ravel('F'), loc="upper center", bbox_to_anchor=(0.5, 0.99), ncol=2,fontsize=20)
