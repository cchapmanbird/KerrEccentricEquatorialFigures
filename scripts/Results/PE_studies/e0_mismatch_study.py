...
# Production runs 
eps_vec = np.arange(-10,-1.0,-1.0) 
e0_vec = np.arange(0.1,0.8,0.1)
...
k = 0
mismatch_vals = np.zeros((len(eps_vec), len(e0_vec)))
for M_mu in tqdm(M_mu_vec):
    # Initialize heat map storage
    for i, e_val in (enumerate(e0_vec)):

        # Compute semi-latus rectum giving plunge for T = 4 years for each e0
        traj_args = [M_mu_vec[0], M_mu_vec[1], 0.998, e_val, 1.0]
        index_of_p = 3
        # Check to see what value of semi-latus rectum is required to build inspiral lasting T years. 
        p_plunge = get_p_at_t(
            traj,
        ...
        params = [M_mu_vec[0],M_mu_vec[1], 0.998, p_plunge, e_val, 1.0, dist, qS, phiS, qK, phiK, Phi_phi0, Phi_theta0, Phi_r0]
        ...
        for j, eps_val in enumerate(eps_vec):
            ...
            mismatch_vals[i,j] = xp.asnumpy(mismatch_function(EMRI_TDI_perturb_e0,Kerr_TDI_waveform_circ, PSD_AET, delta_t,N_channels=2)) 
