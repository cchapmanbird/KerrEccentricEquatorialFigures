...
for k,mass_ratio in enumerate(mass_ratio_vec):
    ...
    plt.semilogx(err_vec, median_phase_diff, lsh, c=cpal[k], markersize=3)
    plt.fill_between(err_vec, median_phase_diff - sigma_phase_diff, median_phase_diff + sigma_phase_diff, alpha=0.3,color=cpal[k])
    
    if k == 0:
        plt.axhline(0, color='k', lw=1., linestyle='-', label='1.0 rad')
        plt.axhline(-1, color='gray', linestyle='--', lw=1., label='0.1 rad')
        plt.ylabel(r'$\log_{10} \Delta \Phi_\phi$')
    ...
    plt.semilogx(err_vec, median_timing, lsh, label=rf'$10^{{{int(np.log10(mass_ratio)):d}}}$',c=cpal[k], markersize=3)
    plt.fill_between(err_vec, median_timing - sigma_timing, median_timing + sigma_timing, alpha=0.3,color=cpal[k])

    if k == 2:
        plt.legend(frameon=False, )
        plt.ylabel('CPU wall-time [ms]')
        plt.tick_params(axis='x', labelbottom=False)
    ...
    plt.semilogx(err_vec, median_N_points, lsh, label=f'mass ratio={mass_ratio}',c=cpal[k], markersize=3)
    ...
    if k == 2:
        plt.xlabel(r'$\sigma_\mathrm{tol}$')
